...
<section class="custom-slider">
    ...
    <div class="flexslider posts-slider loading">
        ...
        <?php while( has_sub_field('custom_add_new_slide' ) ) : ?>
			...
			/** modified from big-size for sidebar layout **/
                        $slide_image = wp_get_attachment_image_src( $attachment_id, $image_size );
                    ?>
                        <img src="<?php echo $slide_image[0]; ?>" width="<?php echo $slide_image[1]; ?>" height="<?php echo $slide_image[2]; ?>" class="attachment-medium-size wp-post-image" alt="<?php the_sub_field( 'custom_slide_title' ); ?>" />
                    <?php } else { ?>
